...
_Примечание: Поддерживаемые `тип`, `категория` и `флаги` перечислены в разделе "Создание журнала изменений"._
