...
2. `entities` (_Опц._ сущности) — бизнес-сущности, которыми **оперирует предметная область** (например, User, Product, Order)
3. `features` (_Опц._ фичи) — взаимодействия с пользователем, **действия, которые несут бизнес-ценность для пользователя.** (например, SendComment, AddToCart, UsersSearch)
...
6. `processes` (_Опц._ процессы, устаревший слой) — сложные сценарии, покрывающие несколько страниц. (например, авторизация)
