...
Добавляя атрибуты #data- _, даже обычные элементы HTML могут стать довольно сложными и мощными программными объектами. Например, «спрайт» космического корабля в игре может быть простым элементом [`<img>`](https://developer.mozilla.org/ru/docs/Web/HTML/Element/img) с атрибутом класса и несколькими атрибутами data- _:
