...
$element.addEventListener(
  ...
  function (e) {
    ...
    console.log(
      `Элемент, для которого запущен обработчик: <${e.currentTarget.tagName.toLowerCase()}>`,
    );
    console.log(
      `Элемент, который инициировал событие click: <${e.target.tagName.toLowerCase()}>`,
    );
  },
  true,
);
...
$elements.forEach(function ($element) {
  // на фазе всплытия
  `$element.addEventListener('click', log);` // на фазе погружения
