...
info:
  - >-
    [[0040 Тернарный оператор и конструкция SwitchCase|Тернарный оператор и
...
function createOAuthString(host, clientId, scope) {
  return (
    host + "/login/oauth/authorize?client_id=" + clientId + "&scope=" + scope
  );
}
...
function bold(strings, ...expressions) {
  let finalString = ""; // Проходимся по всем выражениям
...
```html
This is a string with <strong>true</strong> and <strong>false</strong> and
<strong>100</strong> interpolated inside.
```
