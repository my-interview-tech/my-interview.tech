...
function freeze(value) {
  console.log(value);
  return Promise.resolve(value + 1).then(freeze); // Здесь асинхронная операция
...
const emulateWork = (name, time) => () => {
  ...
  return new Promise((resolve) => {
    setTimeout(() => console.log(`End ${name}`) || resolve(), time);
  });
};
