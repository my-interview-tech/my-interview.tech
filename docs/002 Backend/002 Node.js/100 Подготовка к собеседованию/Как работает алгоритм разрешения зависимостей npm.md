...
6. npm установит **`packageA`** и **`packageB`**, так как они требуются в проекте.
7. npm обнаружит, что оба пакета зависят от **`lodash`**, но с разными версиями.
8. npm создаст структуру:
    - Установит **`lodash@4.17.10`** для **`packageA`**.
    - Установит **`lodash@4.17.15`** для **`packageB`**.
9. В результате, в директории **`node_modules`** будет две версии **`lodash`**:
