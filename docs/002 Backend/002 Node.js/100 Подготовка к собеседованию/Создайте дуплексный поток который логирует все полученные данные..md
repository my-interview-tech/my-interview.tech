...
tags:
  ...
  - "#обработка-данных"
info:
  - "[Документация по потокам в Node.js](https://nodejs.org/api/stream.html)"
  - "[Руководство по использованию потоков в Node.js](https://nodejs.org/en/docs/guides/backpressuring-in-streams/)"
  - "[Transform потоки в Node.js](https://nodejs.org/api/stream.html#class-streamtransform)"
---
...
class LoggingStream extends Transform {
  _transform(chunk, encoding, callback) {
    console.log("Лог:", chunk.toString())
    this.push(chunk) // Передаем данные дальше
    callback()
  }
}
...
```javascript
const fs = require("fs")
const LoggingStream = require("./LoggingStream")

const readStream = fs.createReadStream("input.txt")
const writeStream = fs.createWriteStream("output.txt")
const logStream = new LoggingStream()

readStream.pipe(logStream).pipe(writeStream)
```
