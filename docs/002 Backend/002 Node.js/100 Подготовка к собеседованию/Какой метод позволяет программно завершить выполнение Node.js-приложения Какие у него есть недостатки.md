---
title: Какой метод позволяет программно завершить выполнение Node.js-приложения Какие у него есть недостатки
draft: true
tags: NodeJS
info:
---
...
`async function shutdown() {   await closeConnections(); // Закрываем соединения   console.log('Все задачи завершены. Закрываем приложение');   process.exit(0); }  shutdown();`
