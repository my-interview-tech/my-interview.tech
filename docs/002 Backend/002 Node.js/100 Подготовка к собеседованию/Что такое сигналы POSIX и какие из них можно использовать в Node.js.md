...
3. **`SIGQUIT`**
    ...
    - Отправляется при нажатии *_Ctrl+*_ в терминале.
...
`process.on('SIGINT', () => {   console.log('Получен сигнал SIGINT (Ctrl+C). Завершаем приложение...');   process.exit(0);  // Завершаем приложение с кодом 0 });`
...
`process.on('SIGTERM', () => {   console.log('Получен сигнал SIGTERM. Завершаем приложение...');   process.exit(0);  // Завершаем приложение с кодом 0 });`
...
`process.on('SIGUSR1', () => {   console.log('Получен сигнал SIGUSR1. Выполняем пользовательскую задачу...');   // Здесь можно выполнить перезагрузку конфигурации или другие действия });`
