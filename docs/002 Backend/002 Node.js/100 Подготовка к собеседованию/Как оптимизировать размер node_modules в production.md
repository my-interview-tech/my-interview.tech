...
- Пример Dockerfile для продакшн-сборки:
    ...
    `# Стартовая стадия FROM node:14 AS build  WORKDIR /app  COPY package*.json ./ RUN npm install --production  COPY . .  # Финальная стадия (только для продакшн) FROM node:14-slim  WORKDIR /app  COPY --from=build /app /app  CMD ["node", "app.js"]`
