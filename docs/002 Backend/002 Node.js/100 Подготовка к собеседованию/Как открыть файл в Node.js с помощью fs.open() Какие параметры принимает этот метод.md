...
2. **`flags` (строка)**:
    ...
    Более подробную информацию о флагах можно найти в документации Node.js.
...
`const fs = require('fs');  // Открытие файла для чтения (если файл существует) fs.open('example.txt', 'r', (err, fd) => {     if (err) {         console.error('Ошибка при открытии файла:', err);         return;     }      console.log('Файл успешно открыт. Файловый дескриптор:', fd);      // Закрытие файла после работы с ним     fs.close(fd, (err) => {         if (err) {             console.error('Ошибка при закрытии файла:', err);         } else {             console.log('Файл успешно закрыт');         }     }); });`
