---
title: Чем удобен axios по сравнению с встроенным модулем https
draft: false
tags:
  - "#NodeJS"
  - "#axios"
...
info:
  ...
  - "[Документация https модуля в Node.js](https://nodejs.org/api/https.html)"
  - "[Сравнение HTTP-клиентов в Node.js](https://www.redhat.com/en/topics/api/what-is-a-rest-api)"
  - "[Использование async/await с HTTP-запросами](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Statements/async_function)"
---
...
- **Axios** автоматически обрабатывает ответы с типом `application/json` и преобразует их в объект JavaScript, что избавляет от необходимости вручную парсить тело ответа, как в случае с `https` и `JSON.parse()`.
  ...
  axios
    ...
    .then((response) => {
      console.log(response.data) // данные уже в виде объекта
    })
    .catch((error) => {
      console.error("Error:", error)
    })
  ...
  https
    .get("https://api.example.com/data", (res) => {
      ...
      res.on("end", () => {
        const parsedData = JSON.parse(data) // необходимо вручную парсить ответ
...
- **Axios** автоматически обрабатывает ошибки HTTP-статусов, отличных от 2xx (например, 404, 500), и позволяет легко получать доступ к статусу и сообщению об ошибке.
  ...
  axios
    ...
    .catch((error) => {
      if (error.response) {
        // Ошибка ответа (например, 404 или 500)
