...
`const fs = require('fs');  // Открытие файла для чтения fs.open('example.txt', 'r', (err, fd) => {     if (err) throw err;     console.log('Файл открыт для чтения:', fd); });  // Открытие файла для записи (с удалением существующего содержимого) fs.open('example.txt', 'w', (err, fd) => {     if (err) throw err;     console.log('Файл открыт для записи:', fd); });  // Открытие файла для добавления данных в конец fs.open('example.txt', 'a', (err, fd) => {     if (err) throw err;     console.log('Файл открыт для добавления:', fd); });`
